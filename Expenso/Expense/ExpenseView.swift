        ...
        NavigationView {
            ZStack {
                ...
                VStack {
                    ...
                    ToolbarModelView(title: "Balance", hasBackButt: false, button1Icon: IMAGE_OPTION_ICON, button2Icon: IMAGE_FILTER_ICON) { self.presentationMode.wrappedValue.dismiss() }
                        ...
                        .actionSheet(isPresented: $showFilterSheet) {
                            ActionSheet(title: Text("Select a filter"), buttons: [
                                    ...
                                    .default(Text("Last Month")) { filter = .lastmonth },
                                    .cancel()
                            ])
                        }
...
struct AppleCardViewWrapper: UIViewControllerRepresentable {
    var inbound: String
    var outbound: String
    var balance: String
    let appleCardView = AppleCardView()
    
    func makeUIViewController(context: Context) -> AppleCardView {
        appleCardView.inbound = inbound
        appleCardView.outbound = outbound
        appleCardView.balance = balance
        return appleCardView
    }

    func updateUIViewController(_ uiViewController: AppleCardView, context: Context) {
    ...
    var body: some View {
        ...
        ScrollView(showsIndicators: false) {
            ...
            if fetchRequest.wrappedValue.isEmpty {
                ...
                VStack() {
                    let a = { ExpenseModelView(isIncome: true, filter: filter) }()
                    let b =  { ExpenseModelView(isIncome: false, filter: filter) }()
                    AppleCardViewWrapper(inbound: "\(a.CURRENCY)\(a.getTotalValue())", outbound: "-\(b.CURRENCY)\(b.getTotalValue())", balance: "\(CURRENCY)\(getTotalBalance())")
                }.frame(height: UIScreen.main.bounds.size.width * 0.94 * (2.0 / 3.0) + 10)
                HStack(spacing: 8) {
                ...
                }.frame(maxWidth: UIScreen.main.bounds.size.width * 0.94)
...
struct ExpenseModelView: View {
    ...
    var body: some View {
        ...
        }.padding(.bottom, 12).background(Color.secondary_color).cornerRadius(12)
    }
}
...
struct ExpenseTransView: View {
    ...
    var body: some View {
        HStack {
            ...
            NavigationLink(destination: NavigationLazyView(ExpenseFilterView(categTag: expenseObj.tag)), label: {
                Image(getTransTagIcon(transTag: expenseObj.tag ?? ""))
                    ...
                    .background(Color.primary_color).cornerRadius(12)
            })
            
            let amount = String(format: "%.2f", expenseObj.amount)
            VStack(alignment: .leading, spacing: 6) {
                HStack {
                    TextView(text: expenseObj.title ?? "", type: .subtitle_1, lineLimit: 1).foregroundColor(Color.text_primary_color)
                    Spacer()
                    TextView(text: "\(expenseObj.type == TRANS_TYPE_INCOME ? "+" : "-")\(CURRENCY)\(amount)", type: .subtitle_1)
        ...
        }.padding(8).background(Color.secondary_color).cornerRadius(12)
    }
}
